...
COMMENT ON EXTENSION pgturbohybrid IS 'TurboHybrid core retrieval index access method for pgvector dense vector and BM25 search. [stable public]';
COMMENT ON ACCESS METHOD turbohybrid IS 'TurboHybrid core index access method (dense vector + BM25). [stable public]';
...
COMMENT ON FUNCTION turbohybrid_query(vector, pg_catalog.tsquery, pg_catalog.text, pg_catalog.float8, pg_catalog.float8, pg_catalog.float8, pg_catalog.int4, pg_catalog.int4, pg_catalog.int4, pg_catalog.int4, pg_catalog.bool) IS 'Construct a core TurboHybrid query payload for dense and/or BM25 retrieval with fusion. [stable public]';
...
-- Mechanical, read-only index integrity diagnostics.
...
COMMENT ON FUNCTION turbohybrid_validate_index(regclass, boolean) IS 'Mechanically validate pgturbohybrid page chains, tuples, branches, and optional deep graph reachability without modifying the index. [diagnostic]';
