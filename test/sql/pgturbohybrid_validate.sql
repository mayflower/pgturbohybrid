...
CREATE TABLE validate_sparse (id int, s turbohybrid_sparse_vector, body tsvector);
INSERT INTO validate_sparse VALUES
  (1, turbohybrid_sparse_vector_build(ARRAY[1]::int4[], ARRAY[1.0]::float4[]),
      to_tsvector('simple', 'alpha beta')),
  (2, turbohybrid_sparse_vector_build(ARRAY[2]::int4[], ARRAY[2.0]::float4[]),
      to_tsvector('simple', 'beta gamma'));
CREATE INDEX validate_sparse_idx ON validate_sparse USING turbohybrid
  (s sparse_ip_turbohybrid_ops, body bm25_tsvector_turbohybrid_ops)
  WITH (sparse_quant_bits = 0);

DO $$
DECLARE
  result jsonb := turbohybrid_validate_index('validate_sparse_idx'::regclass, true);
BEGIN
  IF NOT (result->>'ok')::boolean OR (result->>'live_nodes')::int <> 2 OR
     result->'branches'->'sparse'->>'status' <> 'valid' OR
     result->'branches'->'bm25'->>'status' <> 'valid' THEN
    RAISE EXCEPTION 'unexpected sparse-primary validation: %', result;
  END IF;
END $$;

DROP TABLE validate_dense;
DROP TABLE validate_sparse;
