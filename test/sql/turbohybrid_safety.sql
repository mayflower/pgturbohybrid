...
BEGIN
	...
	FROM (
		...
		ORDER BY embedding <~> hybrid_query(
			text_query => to_tsquery('english', 'large'),
			dense_k => 0,
			bm25_k => 5,
			final_k => 5
		)
	...
	FROM (
		...
		FROM th_safety_hybrid
		ORDER BY embedding <~> hybrid_query(
			text_query => to_tsquery('english', 'missing'),
			dense_k => 0,
			bm25_k => 5,
			final_k => 5
		)
	...
	IF large_count <> 1 OR missing_count <> 0 THEN
		RAISE EXCEPTION 'unexpected BM25 counts: large %, missing %',
			large_count, missing_count;
	END IF;
END
$$;
...
BEGIN
	...
	IF top_id <> 4 THEN
		RAISE EXCEPTION 'expected delta BM25 top-1 id 4, got %', top_id;
	END IF;
END
$$;
