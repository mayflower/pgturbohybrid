...
BEGIN
	...
	))::text != 'turbohybrid_query(fusion=weighted,vector=false,tsquery=true,dense_weight=1,bm25_weight=1,alpha=0.25,rrf_k=60,dense_k=400,bm25_k=400,final_k=10,require_bm25_match=true)' THEN
...
DECLARE
	ok bool;
BEGIN
	SELECT bool_and(provolatile = 's' AND proparallel = 's') INTO ok
	FROM pg_proc
	WHERE oid IN (
	...
	IF NOT ok THEN
		RAISE EXCEPTION 'pgturbohybrid distance functions should be stable and parallel safe';
	END IF;
END
$$;
