...
BEGIN
	...
	IF current_setting('turbohybrid.multivector_max_doc_vectors') != '256' OR
		...
		current_setting('turbohybrid.multivector_debug_trace_limit') != '1000' THEN
			RAISE EXCEPTION 'unexpected multivector defaults: doc %, query %, subvector %, unique %, raw %, adaptive %, branch_plan %, source %, bm25_injection %, sparse_source %, reservoirs %, coverage_reservoir_k %, fallback %, fallback_fraction %, fallback_max_docs %, per_token_reservoir_k %, docmap %, docs %, doc_graph_entry_sample_count %, doc_graph_search_ef %, doc_graph_oversampling %, doc_graph_rescore_k %, doc_storage %, doc_storage_cache %, rerank %, rerank_k %, allow_exact_symmetric_build %, exact_symmetric_build_max_docs %, model %, proxy_encoder %, accumulator_mb %, debug %, skip_tokens %, trace_limit %',
