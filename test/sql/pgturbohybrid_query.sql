...
BEGIN
	...
	IF current_setting('turbohybrid.multivector_max_doc_vectors') != '256' OR
		...
		current_setting('turbohybrid.multivector_debug_trace_limit') != '1000' THEN
			RAISE EXCEPTION 'unexpected multivector defaults: doc %, query %, subvector %, unique %, raw %, adaptive %, source %, bm25_injection %, reservoirs %, coverage_reservoir_k %, fallback %, fallback_fraction %, fallback_max_docs %, per_token_reservoir_k %, docmap %, docs %, doc_graph_search_ef %, doc_graph_oversampling %, doc_graph_rescore_k %, doc_storage %, rerank %, rerank_k %, accumulator_mb %, debug %, skip_tokens %, trace_limit %',
