...
DROP TABLE IF EXISTS rb_residual;
CREATE TABLE rb_residual (id int PRIMARY KEY, embedding vector(64), body_tsv tsvector);
INSERT INTO rb_residual(id, embedding, body_tsv)
SELECT id, embedding, body_tsv FROM rb_free;
CREATE INDEX rb_residual_idx ON rb_residual
    USING turbohybrid (embedding vector_cosine_turbohybrid_ops, body_tsv bm25_tsvector_turbohybrid_ops)
    WITH (quantization_bits = 4, residual_rerank = on, residual_rerank_bytes = 16);
...
BEGIN
    ...
    -- (2b) Heap-backed exact rescore is explicit and works for exact-free indexes.
    SET turbohybrid.dense_heap_rescore = topk;
    PERFORM id FROM rb_free ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    topk_heap_count := (st->>'heap_rescore_count')::int;
    IF st->>'dense_heap_rescore' <> 'topk' OR topk_heap_count <= 0 OR
    ...
    SET turbohybrid.dense_heap_rescore = band;
    PERFORM id FROM rb_free ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    ...
    -- (2e) Uncertainty retry is a higher-level bounded retry around the graph
    -- traversal. It is off by default for latency, and only runs a second pass
    -- when max_passes permits at least one retry.
    SET turbohybrid.dense_heap_rescore = off;
    SET turbohybrid.dense_adaptive_widening = off;
    ...
    PERFORM id FROM rb_free ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    IF st->>'dense_uncertainty_retry_mode' <> 'on' OR
       (st->>'dense_uncertainty_retry_triggered')::bool IS DISTINCT FROM true OR
       st->>'dense_uncertainty_retry_reason' <> 'forced' OR
       (st->>'dense_uncertainty_retry_passes')::int <> 2 OR
       (st->>'dense_uncertainty_final_target')::int <=
           (st->>'dense_uncertainty_initial_target')::int OR
       (st->'dense'->'uncertainty_retry'->>'triggered')::bool IS DISTINCT FROM true THEN
        RAISE EXCEPTION 'forced dense uncertainty retry stats unexpected: %', st;
    END IF;

    SET turbohybrid.dense_uncertainty_retry = auto;
    SET turbohybrid.dense_uncertainty_min_gap = 1.0;
    PERFORM id FROM rb_free ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    IF st->>'dense_uncertainty_retry_mode' <> 'auto' OR
       (st->>'dense_uncertainty_retry_triggered')::bool IS DISTINCT FROM true OR
       st->>'dense_uncertainty_retry_reason' NOT IN
           ('flat_top10', 'flat_boundary', 'underfilled', 'payload_underfilled') OR
       (st->>'dense_uncertainty_retry_passes')::int <> 2 THEN
        RAISE EXCEPTION 'auto dense uncertainty retry stats unexpected: %', st;
    END IF;
    ...
    SET turbohybrid.dense_heap_rescore = off;
    SET turbohybrid.dense_residual_rerank_mode = fixed;
    ...
    IF st->>'exact_rescore_source' <> 'residual' OR
       (st->>'dense_residual_rerank_bytes')::int <> 16 OR
       (st->>'dense_residual_rerank_count')::int <= 0 OR
       st->>'residual_rerank_mode' <> 'fixed' OR
       (st->>'residual_rerank_weight_effective')::float8 < 0.029 OR
       (st->>'residual_rerank_weight_effective')::float8 > 0.031 OR
       (st->>'residual_rerank_band')::int <> (st->>'dense_residual_rerank_count')::int THEN
        RAISE EXCEPTION 'fixed residual rerank did not report expected stats: %', st;
    END IF;
    IF (st->>'exact_rescore_count')::int <> 0 OR
       (st->>'heap_rescore_count')::int <> 0 THEN
        RAISE EXCEPTION 'residual rerank should not report exact/heap counts: %', st;
    END IF;

    SET turbohybrid.dense_residual_rerank_mode = calibrated;
    SET turbohybrid.dense_residual_rerank_weight = -1;
    SET turbohybrid.dense_residual_rerank_max_adjust_ratio = 0.15;
    PERFORM id FROM rb_residual ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    IF st->>'exact_rescore_source' <> 'residual' OR
       st->>'residual_rerank_mode' <> 'calibrated' OR
       (st->>'residual_rerank_weight_effective')::float8 < 0 OR
       (st->>'residual_rerank_band')::int <= 0 OR
       (st->>'residual_rerank_reordered_count')::int < 0 OR
       st->>'residual_rerank_max_adjustment' IN ('NaN', 'Infinity', '-Infinity') OR
       (st->>'residual_rerank_max_adjustment')::float8 < 0 THEN
        RAISE EXCEPTION 'calibrated residual rerank did not report expected stats: %', st;
    END IF;

    -- A zero calibrated clamp is a flat/no-adjustment guard: it must not
    -- produce NaN/inf and should leave the maximum applied adjustment at zero.
    SET turbohybrid.dense_residual_rerank_max_adjust_ratio = 0;
    PERFORM id FROM rb_residual ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    IF st->>'residual_rerank_mode' <> 'calibrated' OR
       st->>'residual_rerank_max_adjustment' IN ('NaN', 'Infinity', '-Infinity') OR
       (st->>'residual_rerank_max_adjustment')::float8 <> 0 THEN
        RAISE EXCEPTION 'zero-clamp calibrated residual rerank stats unexpected: %', st;
    END IF;

    SET turbohybrid.dense_residual_rerank_mode = off;
    PERFORM id FROM rb_residual ORDER BY embedding <~> turbohybrid_query(vector_query => qv) LIMIT 10;
    st := turbohybrid_last_scan_stats();
    IF st->>'exact_rescore_source' <> 'none' OR
       st->>'residual_rerank_mode' <> 'off' OR
       (st->>'dense_residual_rerank_count')::int <> 0 OR
       (st->>'residual_rerank_band')::int <> 0 THEN
        RAISE EXCEPTION 'residual rerank off mode did not skip sketches: %', st;
    END IF;

    RESET turbohybrid.dense_residual_rerank_mode;
    RESET turbohybrid.dense_residual_rerank_weight;
    RESET turbohybrid.dense_residual_rerank_max_adjust_ratio;
    SET turbohybrid.dense_heap_rescore = auto;
    PERFORM set_config('turbohybrid.profile', 'latency', true);
    ...
    END IF;
    IF st->>'exact_rescore_source' <> 'index_exact' THEN
        RAISE EXCEPTION 'exact_storage rescore source not index_exact: %', st;
    END IF;

    -- (5) exact_storage index: band = off disables exact rescore.
    ...
    -- (6) Hybrid (vector + bm25) scan on the exact-free index under the latency
...
DROP INDEX rb_residual_idx;
DROP TABLE rb_free;
DROP TABLE rb_exact;
DROP TABLE rb_residual;
