...
BEGIN
	stats := turbohybrid_index_stats('tqh_segment_docs_idx'::regclass);
	IF (stats->>'live_nodes')::int <> 5 OR
		(stats->>'dead_nodes')::int < 1 OR
		(stats->>'live_entry_node')::bigint < 0 OR
		NOT stats ? 'dead_bridge_nodes' OR
		NOT stats ? 'avg_live_degree_level0' OR
		NOT stats ? 'dead_neighbor_refs' OR
		NOT stats ? 'reindex_recommended' OR
		NOT stats ? 'reindex_reason' THEN
		RAISE EXCEPTION 'unexpected native vacuum diagnostics: %', stats;
	END IF;
...
-- Native VACUUM must preserve routing across deleted entry/hub candidates and
-- append-only churn.  Compare the graph result with an exact heap-vector oracle
-- for fixed queries after two mutation cycles.
CREATE TABLE tqh_vacuum_topology_docs (
	id int PRIMARY KEY,
	embedding vector(2) NOT NULL
);
INSERT INTO tqh_vacuum_topology_docs
SELECT g, format('[%s,%s]', g, g % 7)::vector
FROM generate_series(1, 64) AS g;
CREATE INDEX tqh_vacuum_topology_idx ON tqh_vacuum_topology_docs
USING turbohybrid (embedding vector_l2_turbohybrid_ops)
WITH (graph_m = 4, graph_ef_construction = 64, graph_ef_search = 128,
	  exact_storage = on);
DELETE FROM tqh_vacuum_topology_docs
WHERE id IN (1, 2, 3, 16, 17, 31, 32, 33, 48, 63, 64);
VACUUM tqh_vacuum_topology_docs;
INSERT INTO tqh_vacuum_topology_docs
SELECT g, format('[%s,%s]', g, g % 7)::vector
FROM generate_series(65, 80) AS g;
DELETE FROM tqh_vacuum_topology_docs WHERE id % 11 = 0;
...
DECLARE
	...
	stats jsonb;
BEGIN
	FOREACH query IN ARRAY ARRAY['[4,4]'::vector, '[29,1]'::vector,
								 '[70,0]'::vector]
	LOOP
		SET LOCAL enable_indexscan = off;
		SELECT id INTO exact_id
		FROM tqh_vacuum_topology_docs
		ORDER BY embedding <-> query, id
		LIMIT 1;
		SET LOCAL enable_indexscan = on;
		SET LOCAL enable_seqscan = off;
		SELECT id INTO graph_id
		FROM tqh_vacuum_topology_docs
		ORDER BY embedding <~-> turbohybrid_query(
			vector_query => query, dense_k => 32, final_k => 1)
		LIMIT 1;
		IF graph_id IS DISTINCT FROM exact_id THEN
			RAISE EXCEPTION 'post-vacuum graph/oracle mismatch for %: graph %, exact %',
				query, graph_id, exact_id;
		END IF;
	END LOOP;
	stats := turbohybrid_index_stats('tqh_vacuum_topology_idx'::regclass);
	IF (stats->>'live_nodes')::int <> (SELECT count(*) FROM tqh_vacuum_topology_docs) OR
		(stats->>'dead_nodes')::int = 0 OR
		(stats->>'live_entry_node')::int < 0 THEN
		RAISE EXCEPTION 'unexpected churn topology stats: %', stats;
	END IF;
END
$$;

DROP TABLE tqh_vacuum_topology_docs;
...
BEGIN
	...
	IF NOT (keys <@ ARRAY[
		...
		'build_edges_us',
		'build_fast_edges',
		'build_memory_centroid_clustering_bytes',
		'build_memory_centroid_residual_bytes',
		'build_memory_graph_edge_bytes',
		'build_memory_heap_scan_decode_bytes',
		'build_memory_page_wal_bytes',
		'build_memory_posting_tuple_bytes',
		'build_memory_proxy_encoding_bytes',
		'build_memory_sidecar_tuple_bytes',
		'build_memory_token_pooling_bytes',
		'build_neighbor_select',
		'build_neighbor_select_reason',
		'build_peak_memory_context_bytes',
		'build_peak_memory_phase',
		'connect_backbone_us',
		'dense_build_distance_mode',
		'dimensions',
		'ef_construction',
		...
		'graph_neighbor_bytes_estimate',
		'graph_node_bytes_estimate',
		'index_shape',
		'm',
		'multivector_centroid_build_docs',
		'multivector_centroid_build_us',
		'multivector_centroid_build_vectors',
		'multivector_centroid_cluster_us',
		'multivector_centroid_posting_bytes',
		'multivector_centroid_posting_count',
		'multivector_centroid_posting_write_us',
		'multivector_centroid_residual_bytes',
		'multivector_centroid_residual_us',
		'multivector_centroid_sidecar_write_us',
		'multivector_centroid_vector_bytes',
		'multivector_doc_exact_build_distance_calls',
		'multivector_doc_exact_build_distance_us',
		'multivector_doc_sidecar_write_us',
		'multivector_doc_vector_chunk_ref_bytes',
		'multivector_doc_vectors_pointer_bytes',
		'multivector_docmap_bytes_estimate',
		...
		'native_segments',
		'node_count',
		'parallel_edge_build_disabled_reason',
	...
	IF NOT (ARRAY[
		...
		'multivector_plain_fallback_pairs',
			...
			'multivector_proxy_graph_searches',
		...
		'version'
			...
			RAISE EXCEPTION 'missing required pgturbohybrid last scan stats keys: %', keys;
		END IF;
...
BEGIN
	...
	PERFORM set_config('turbohybrid.multivector_adaptive_widening', 'auto', true);
	IF current_setting('turbohybrid.multivector_adaptive_widening') <> 'auto' THEN
		RAISE EXCEPTION 'multivector_adaptive_widening auto GUC did not stick';
	END IF;

	PERFORM set_config('turbohybrid.multivector_adaptive_widening', 'on', true);
	IF current_setting('turbohybrid.multivector_adaptive_widening') <> 'on' THEN
		RAISE EXCEPTION 'multivector_adaptive_widening on GUC did not stick';
	END IF;
	...
	IF current_setting('turbohybrid.multivector_docmap') <> 'auto' THEN
		RAISE EXCEPTION 'multivector_docmap auto GUC did not stick';
	END IF;

	PERFORM set_config('turbohybrid.multivector_docmap', 'require', true);
	IF current_setting('turbohybrid.multivector_docmap') <> 'require' THEN
		RAISE EXCEPTION 'multivector_docmap require GUC did not stick';
	END IF;

	PERFORM set_config('turbohybrid.multivector_docmap', 'off', true);
	IF current_setting('turbohybrid.multivector_docmap') <> 'off' THEN
		RAISE EXCEPTION 'multivector_docmap off GUC did not stick';
	END IF;
