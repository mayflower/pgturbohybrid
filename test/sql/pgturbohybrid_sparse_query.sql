...
SELECT turbohybrid_experimental_query(
...
SELECT turbohybrid_experimental_query(vector_query => '[1,2,3]'::vector)::text AS no_sparse;
...
SELECT id,
       round((s <~*> turbohybrid_experimental_query(
...
SELECT turbohybrid_experimental_query(require_sparse_match => true);
...
SELECT turbohybrid_sparse_vector_build(ARRAY[1]::int4[], ARRAY[1.0]::float4[])
       <~*> turbohybrid_experimental_query(vector_query => '[1]'::vector);
