...
CREATE OR REPLACE FUNCTION pg_temp.perf_plan_root(plan_doc jsonb)
RETURNS jsonb
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION pg_temp.perf_plan_nodes(plan_node jsonb)
RETURNS SETOF jsonb
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    LOOP
        RETURN QUERY SELECT * FROM pg_temp.perf_plan_nodes(child);
    END LOOP;
END;
$$;

CREATE OR REPLACE FUNCTION pg_temp.perf_plan_has_seq_scan(plan_node jsonb)
RETURNS bool
LANGUAGE sql
AS $$
    SELECT EXISTS (
        SELECT 1
        FROM pg_temp.perf_plan_nodes(plan_node) AS n(node)
        WHERE n.node->>'Node Type' = 'Seq Scan'
    )
$$;

CREATE OR REPLACE FUNCTION pg_temp.perf_plan_has_top_sort_over_seq_scan(plan_node jsonb)
RETURNS bool
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    IF node_type IN ('Sort', 'Incremental Sort') THEN
        RETURN pg_temp.perf_plan_has_seq_scan(plan_node);
    END IF;
    ...
    IF node_type IN ('Limit', 'Result', 'Gather', 'Gather Merge') THEN
        FOR child IN
            ...
            FROM jsonb_array_elements(COALESCE(plan_node->'Plans', '[]'::jsonb))
        LOOP
            IF pg_temp.perf_plan_has_top_sort_over_seq_scan(child) THEN
                RETURN true;
            END IF;
        END LOOP;
    END IF;
...
CREATE OR REPLACE FUNCTION pg_temp.perf_assert_turbohybrid_index_plan(
    plan_doc jsonb,
    expected_index regclass,
    query_label text DEFAULT 'pgturbohybrid benchmark query')
RETURNS jsonb
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    IF expected_index_name IS NULL THEN
        RAISE EXCEPTION USING
            ERRCODE = '23514',
            MESSAGE = format('expected pgturbohybrid index does not exist for %s', query_label);
    END IF;
    ...
    root := pg_temp.perf_plan_root(plan_doc);
    ...
    SELECT EXISTS (
        SELECT 1
        FROM pg_temp.perf_plan_nodes(root) AS n(node)
        WHERE n.node->>'Node Type' = 'Index Scan'
          AND n.node->>'Index Name' = expected_index_name
    )
    INTO has_expected_index_scan;

    top_sort_seq := pg_temp.perf_plan_has_top_sort_over_seq_scan(root);

    IF NOT has_expected_index_scan THEN
        RAISE EXCEPTION USING
            ERRCODE = '23514',
            MESSAGE = format('benchmark query %s did not use Index Scan on %s',
    ...
    IF top_sort_seq THEN
        RAISE EXCEPTION USING
            ERRCODE = '23514',
            MESSAGE = format('benchmark query %s used a top-level Sort over Seq Scan',
                             query_label),
            DETAIL = left(plan_doc::text, 4000);
    END IF;
