...
-- can improve build time, but query search must seed all segments and merge the
-- resulting candidate set, so latency and recall should be checked together.
...
\echo 'rows=' :NROWS ' dims=' :DIMS ' qset=' :QSET ' warm=' :WARM ' timed=' :TIMED ' final_k=' :FINAL_K ' dense_k=' :DENSE_K ' segments=' :SEGMENTS
...
CREATE TEMP TABLE th_segments_config(segment_count int PRIMARY KEY);
INSERT INTO th_segments_config(segment_count)
SELECT trim(value)::int
...
SELECT pg_temp.th_segments_run(segment_count, :WARM, :TIMED, :DENSE_K,
                               :FINAL_K, :BITS, :QSET)
...
\echo '== raw stats =='
SELECT segment_count, index_stats, last_scan_stats
FROM th_segments_results
ORDER BY segment_count;
