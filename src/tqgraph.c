...
static bool
TqGraphBuildVectorsEqual(TqGraphBuildState *state, uint32 left, uint32 right)
{
	Vector	   *leftVector;
	Vector	   *rightVector;
	Size		vectorSize;
	...
	vectorSize = VECTOR_SIZE(leftVector->dim);
	return memcmp(leftVector, rightVector, vectorSize) == 0;
}

static uint64
TqGraphBuildVectorHash(Vector *vector)
{
	const unsigned char *bytes = (const unsigned char *) vector;
	Size		vectorSize = VECTOR_SIZE(vector->dim);
					   ...
					   Datum *values, bool *isnull)
{
	...
	vectorHash = TqGraphBuildVectorHash(vector);
...
TqGraphFitCorrection(TqGraphBuildState *state)
{
	...
	for (uint32 row = 0; row < state->nodeCount;)
	{
		...
		while (runEnd < state->nodeCount &&
			   TqGraphBuildVectorsEqual(state, row, runEnd))
		{
			if ((runEnd & 0x3FF) == 0)
				CHECK_FOR_INTERRUPTS();
			runEnd++;
...
{
	Size		codeBytes = TqGraphCodeBytesForBits(state->dimensions,
													state->tqBits);
	...
	for (uint32 row = 0; row < state->nodeCount; row++)
	{
		...
		CHECK_FOR_INTERRUPTS();
		if (row > 0 && TqGraphBuildVectorsEqual(state, row - 1, row))
		{
			TqGraphBuildNode *prev = &state->nodes[row - 1];
...
static void
TqGraphAddNeighborIfRoom(TqGraphBuildState *state, uint32 src, uint32 dst,
						 int level)
{
	TqGraphBuildNode *node = &state->nodes[src];
	int			maxNeighbors = TqGraphLevelM(state->m, level);
...
static uint32
TqGraphAdjacentDuplicateCount(TqGraphBuildState *state)
{
	uint32		duplicates = 0;

	for (uint32 i = 1; i < state->nodeCount; i++)
	{
		if (TqGraphBuildVectorsEqual(state, i - 1, i))
			duplicates++;
	}
...
static void
TqGraphLinkAdjacentBuildNode(TqGraphBuildState *state, uint32 nodeId,
							 uint32 prevId)
{
	TqGraphBuildNode *node = &state->nodes[nodeId];

	if (!TqGraphBuildNodeHasLevel(state, nodeId, 0) ||
		!TqGraphBuildNodeHasLevel(state, prevId, 0))
		return;

	node->neighbors[0][0] = prevId;
	node->neighborCounts[0] = 1;
	TqGraphAddNeighborIfRoom(state, prevId, nodeId, 0);
}
...
TqGraphBuildEdges(TqGraphBuildState *state)
{
	...
	adjacentDuplicates = TqGraphAdjacentDuplicateCount(state);
	preserveScanOrder = adjacentDuplicates > state->nodeCount / 2;
	elog(DEBUG1, "turboquant native graph duplicate-run build: nodes=%u adjacent_duplicates=%u preserve_scan_order=%s",
	...
	for (uint32 orderIdx = 1; orderIdx < state->nodeCount; orderIdx++)
	{
		...
		int			linkingLevel = Min(nodeLevel, entryLevel);
			...
			{
				TqGraphLinkAdjacentBuildNode(state, i, i - 1);
					  ...
					  int minResultTarget)
{
	...
	HnswPrepareTqQueryWithBits(scan->indexRelation, &so->support, query,
							   &so->tq,
							   meta.tqBits != 0 ? meta.tqBits : TQ_DEFAULT_BITS);
