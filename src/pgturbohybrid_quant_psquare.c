...
{
	double		target_probabilities[PGTURBOHYBRID_PSQUARE_N];
	double		n_desired[PGTURBOHYBRID_PSQUARE_N];
	...
	if (p->count < PGTURBOHYBRID_PSQUARE_N)
	{
		p->heights[p->count] = x;
		p->count++;
		if (p->count == PGTURBOHYBRID_PSQUARE_N)
		{
			...
			for (int i = 0; i < PGTURBOHYBRID_PSQUARE_N - 1; i++)
			{
				for (int j = i + 1; j < PGTURBOHYBRID_PSQUARE_N; j++)
				{
			...
			p->n_positions[PGTURBOHYBRID_PSQUARE_N - 1] = (double) PGTURBOHYBRID_PSQUARE_N;
		}
		return;
	}
	...
	else if (x > p->heights[PGTURBOHYBRID_PSQUARE_N - 1])
	{
		p->heights[PGTURBOHYBRID_PSQUARE_N - 1] = x;
		k = PGTURBOHYBRID_PSQUARE_N - 1;
	}
	else
	{
		k = 0;
		for (int i = 1; i < PGTURBOHYBRID_PSQUARE_N; i++)
		{
			if (x < p->heights[i])
			{
				k = i - 1;
		...
		if (k >= PGTURBOHYBRID_PSQUARE_N - 1)
			k = PGTURBOHYBRID_PSQUARE_N - 2;
	}

	/* Step 2: increment n_positions for markers above k. */
	for (int i = k + 1; i < PGTURBOHYBRID_PSQUARE_N; i++)
		p->n_positions[i] += 1.0;
...
TqPSquareEstimate(const TqPSquareState *p)
{
	...
	if (p->count < PGTURBOHYBRID_PSQUARE_N)
	{
		double		buf[PGTURBOHYBRID_PSQUARE_N];
	...
	return p->heights[PGTURBOHYBRID_PSQUARE_N / 2];
}
