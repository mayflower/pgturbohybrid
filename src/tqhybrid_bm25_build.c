...
static Size
TqHybridBm25DeltaTermTupleSize(uint16 postingCount, uint32 termBytesLen)
{
	return MAXALIGN(offsetof(TqHybridBm25DeltaTermTupleData, postings) +
					sizeof(TqHybridBm25DeltaTermPosting) * postingCount +
					termBytesLen);
}
						   ...
						   BlockNumber *postingsBlkno, uint32 *postingsBytes)
{
	...
	tuple->encoding = useOffset16 ?
		(useOffset16Soa ?
		 TQHYBRID_BM25_POSTINGS_ENCODING_OFFSET16_SOA :
	...
	if (useOffset16Soa)
	{
		...
		for (uint32 i = 0; i < count32; i++)
		{
			...
			if (!TqHybridBm25CanUseOffset16(tuple->firstNodeId, term->nodeId))
				elog(ERROR, "turbohybrid BM25 postings are not offset16 encodable");
			offset = (uint16) (term->nodeId - tuple->firstNodeId);
			tuple->maxTf = Max(tuple->maxTf, term->tf);
			TqHybridBm25PutUint16(&ptr, offset);
			TqHybridBm25PutUint16(&tfPtr, term->tf);
		}
		ptr = tfPtr;
	}
	else
...
static void
TqHybridBm25InitDeltaDirectory(TqHybridBm25DeltaDirectoryTuple directory)
{
	memset(directory, 0, sizeof(TqHybridBm25DeltaDirectoryTupleData));
	directory->type = TQHYBRID_BM25_DELTA_DIRECTORY_TUPLE_TYPE;
	directory->bucketCount = TQHYBRID_BM25_DELTA_TERM_BUCKETS;
	for (uint16 i = 0; i < TQHYBRID_BM25_DELTA_TERM_BUCKETS; i++)
	{
...
static uint32
TqHybridBm25DeltaDirectoryTermPages(TqHybridBm25DeltaDirectoryTuple directory)
{
	uint32		pages = 0;
...
static BlockNumber
TqHybridBm25WriteDeltaTermDirectory(Relation index,
									TqHybridBm25DeltaDirectoryTuple directory)
{
	BlockNumber start = InvalidBlockNumber;
	BlockNumber insertBlkno = InvalidBlockNumber;

	(void) TqGraphAppendTuple(index, MAIN_FORKNUM, &start,
							  HNSW_PAGE_KIND_TQ_BM25_DELTA_TERM,
							  (Item) directory,
							  MAXALIGN(sizeof(TqHybridBm25DeltaDirectoryTupleData)),
							  HNSW_GRAPH_OP_ELEMENT_INSERT,
							  &insertBlkno);
	return start;
}
...
static void
TqHybridBm25AppendDeltaTermSegments(Relation index,
									TqHybridBm25DeltaTuple delta,
									TqHybridBm25DeltaDirectoryTuple directory,
									Size maxItemSize)
{
	char	   *termBytes = TqHybridBm25DeltaTermBytes(delta);

	for (uint16 i = 0; i < delta->termCount; i++)
	{
		TqHybridBm25DeltaTerm *term = &delta->terms[i];
		TqHybridBm25DeltaTermTuple termTuple;
		Size		termTupleSize;
		uint32		bucketNo;
		TqHybridBm25DeltaTermBucket *bucket;
		...
		termTupleSize = TqHybridBm25DeltaTermTupleSize(1, term->termLen);
		if (termTupleSize > maxItemSize)
			ereport(ERROR,
					(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
					 errmsg("turbohybrid BM25 delta term tuple exceeds page size")));

		termTuple = palloc0(termTupleSize);
		termTuple->type = TQHYBRID_BM25_DELTA_TERM_TUPLE_TYPE;
		...
		memcpy(TqHybridBm25DeltaTermTupleBytes(termTuple),
			   termBytes + term->termOffset, term->termLen);

		bucketNo = TqHybridBm25DeltaTermBucketNo(term->termHash);
						...
						ItemPointer heapTid, Datum tsvectorDatum)
{
	...
	bool		deltaCursorHit;
	TqHybridBm25DeltaDirectoryTupleData deltaDirectory;
	Buffer		metaBuf;
	Page		metaPage;
	TqHybridBm25MetaTuple metaTuple;
	GenericXLogState *xlogState;
	...
	TqHybridBm25InitDeltaDirectory(&deltaDirectory);
	if (BlockNumberIsValid(deltaTermDirectoryBlkno))
		(void) TqHybridBm25ReadDeltaDirectory(index, deltaTermDirectoryBlkno,
											  &deltaDirectory);
	deltaCursorHit =
	...
	TqHybridBm25AppendDeltaTermSegments(index, delta, &deltaDirectory,
										maxItemSize);
	deltaTermDirectoryBlkno =
		TqHybridBm25WriteDeltaTermDirectory(index, &deltaDirectory);
	deltaTermPages = TqHybridBm25DeltaDirectoryTermPages(&deltaDirectory);
		 ...
		 chunkNo++)
	{
		...
		switch (tuple->encoding & TQHYBRID_BM25_POSTINGS_ENCODING_MASK)
		{
			case TQHYBRID_BM25_POSTINGS_ENCODING_OFFSET16:
				(*offset16Count)++;
				break;
			case TQHYBRID_BM25_POSTINGS_ENCODING_OFFSET16_SOA:
				(*offset16SoaCount)++;
				break;
...
tq_debug_bm25_term_stats(PG_FUNCTION_ARGS)
{
	...
	TqHybridBm25ReadPostingEncodingCounts(index, &entry, &offset16Count,
										  &offset16SoaCount, &delta16Count,
										  &varintCount, &tfNormCount,
										  &unknownCount);
	initStringInfo(&json);
	appendStringInfo(&json,
					 "{\"term_id\":%u,"
