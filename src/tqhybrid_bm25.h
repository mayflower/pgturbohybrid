...
#define TQHYBRID_BM25_DELTA_TERM_TUPLE_TYPE	0x68
#define TQHYBRID_BM25_DELTA_DIRECTORY_TUPLE_TYPE 0x69
#define TQHYBRID_BM25_DELTA_TERM_BUCKETS	64
...
#define TQHYBRID_BM25_POSTINGS_ENCODING_OFFSET16_SOA 4
...
typedef struct TqHybridBm25DeltaTermPosting
{
	uint32		nodeId;
	ItemPointerData heaptid;
	uint32		docLen;
	uint16		tf;
	uint16		reserved;
} TqHybridBm25DeltaTermPosting;

typedef struct TqHybridBm25DeltaTermTupleData
{
	uint8		type;
	uint8		reserved1;
	uint16		postingCount;
	...
	uint16		termLen;
	uint16		reserved2;
	TqHybridBm25DeltaTermPosting postings[FLEXIBLE_ARRAY_MEMBER];
	/* followed by term bytes */
} TqHybridBm25DeltaTermTupleData;

typedef TqHybridBm25DeltaTermTupleData *TqHybridBm25DeltaTermTuple;

typedef struct TqHybridBm25DeltaTermBucket
{
	BlockNumber startBlkno;
	BlockNumber tailBlkno;
	uint32		pages;
} TqHybridBm25DeltaTermBucket;

typedef struct TqHybridBm25DeltaDirectoryTupleData
{
	uint8		type;
	uint8		reserved1;
	uint16		bucketCount;
	uint32		reserved2;
	TqHybridBm25DeltaTermBucket buckets[TQHYBRID_BM25_DELTA_TERM_BUCKETS];
} TqHybridBm25DeltaDirectoryTupleData;

typedef TqHybridBm25DeltaDirectoryTupleData *TqHybridBm25DeltaDirectoryTuple;

typedef enum TqHybridBm25DeltaLookupMode
{
	TQHYBRID_BM25_DELTA_LOOKUP_NONE = 0,
	TQHYBRID_BM25_DELTA_LOOKUP_DOC_SCAN,
	TQHYBRID_BM25_DELTA_LOOKUP_TERM_SEGMENT
} TqHybridBm25DeltaLookupMode;
...
const char *TqHybridBm25QueryShapeName(int shape);
const char *TqHybridBm25BooleanEvalModeName(int mode);
const char *TqHybridBm25WandBoundTypeName(int boundType);
const char *TqHybridBm25DeltaLookupModeName(int mode);
