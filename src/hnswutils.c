...
HnswGetEfConstruction(Relation index)
{
	...
	if (HnswIsTurboquantIndex(index))
		return TQ_DEFAULT_GRAPH_EF_CONSTRUCTION;
...
HnswGetTqExactStorageOption(Relation index)
{
	...
	opts = (TqOptions *) index->rd_options;
	return opts != NULL ? opts->tqExactStorage : TQ_DEFAULT_EXACT_STORAGE;
}
						   ...
						   const float *overrideEcScale, int tqBitsOverride)
{
	...
	tq->bits = tqBitsOverride > 0 ? tqBitsOverride : HnswGetTqBits(index);
...
{
	HnswPrepareTqQueryInternal(index, support, value, tq, true, true, NULL, NULL, 0);
}

void
HnswPrepareTqQueryWithBits(Relation index, HnswSupport * support, Datum value,
						   HnswTqQuery *tq, int tqBits)
{
...
{
	HnswPrepareTqQueryInternal(index, support, value, tq, false, true, NULL, NULL, 0);
}
