...
- proxy-vector admission is controlled by the persisted index option
  `multivector_proxy_encoder = mean_pool | max_pool |
  random_projection_fde | learned_projection_placeholder`. `mean_pool`
  preserves the old representative-vector proxy. `max_pool` and
  `random_projection_fde` are pluggable fixed-dimensional encoders for
  admission benchmarks. `learned_projection_placeholder` fails explicitly until
  learned projection weights are supported;
