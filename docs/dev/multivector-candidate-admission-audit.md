...
| `multivector_proxy_encoder` | `normalized_mean` | `normalized_mean`, `mean`, `mean_pool`, `first_token`, `max_abs_mean`, `centroid_mean`, `max_pool`, `random_projection_fde`, `learned_projection_v1` | Document-node index option for the fixed-dimensional proxy key. `normalized_mean` is the production proxy for cosine/IP MaxSim models; `mean_pool` is a compatibility alias for `mean`; `centroid_mean` requires persisted k-means centroids; `max_pool`, `random_projection_fde`, and `learned_projection_v1` are benchmarkable alternatives. `learned_projection_v1` requires configured projection weights. |
